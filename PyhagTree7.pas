...
begin
    cur_pos[1] := penx();
    cur_pos[2] := peny();
    lineto(cur_pos[1] + round(len * cos(angle)), cur_pos[2] + round(len * sin(angle)+4));
end;
...
var
    cur_x, cur_y: integer;
begin
    Window.Title := 'Drooping Alternating Colours Pythagoras Tree';
    if first then
    begin
        moveto(x, y);
